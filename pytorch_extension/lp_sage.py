...
def train_one_step(model, optimizer, loss_fcn, device, feat_len, iter, device_id):
    ...
    out, pos_out, neg_out = batch_pred.split(batch_pred.size(0) // 3, dim=0)
    pos_loss = Func.logsigmoid((out * pos_out).sum(-1)).mean()
    neg_loss = Func.logsigmoid(-(out * neg_out).sum(-1)).mean()
