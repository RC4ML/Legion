...
public:
    ...
    void run(OpParams* params) override {
        ...
        GPU_Random_Sampling(params->stream, graph, cache, memorypool, count, op_id_, is_presc);
        cudaEventRecord(((params->event)), ((params->stream)));
        cudaCheckError();
    }
private:
...
public:
    ...
    void run(OpParams* params) override {
        ...
        get_feature_kernel(params->stream, cache, noder, memorypool, params->device_id, op_id_, in_memory);
    }
private:
    int op_id_;
};
